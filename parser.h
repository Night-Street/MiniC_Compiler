//
// Created by lenovo on 2021/12/03.
//

#ifndef MINI_C_PARSER_H
#define MINI_C_PARSER_H
...
namespace parser {
        ...
        while (1) {
            ...
            for (auto p: productions) {
                ...
                rg(i, 0, p.second.size(), 1) {
                    ...
                    if (i == p.second.size() - 1 ||
                        i < p.second.size() - 1 && next_first.find("") != next_first.end()) {
    ...
    Sign *variable_feature(string type, string id) {
        ...
        if (cur_sign == ";") {
            ...
            if (symbolTable.size() > curScope &&
                find_if(symbolTable[curScope].begin(), symbolTable[curScope].end(), [=](auto s) {
                    return s->name == id;
    ...
    Sign *statement() {
        ...
        } else if (in_first(cur_sign, "expression-statement") &&
                   get_next_sign() != ":") {//以ID开头的语句可能是表达式语句ID=...或标号语句ID:...
    ...
    Sign *if_branch() {
        ...
        if (cur_sign == "if") {
            ...
            ret->code.push_back(
                    new Quadruples("jz", expNode->attributes["addr"], "", "?"));//I.code += (jz, C.add, $, ?)
    ...
    Sign *switch_branch() {
        ...
        if (cur_sign == "switch") {
            ...
            while (cur_sign == "case" || cur_sign == "default") {
                ...
                ret->breakList = SignList::mergeList(ret->breakList,
                                                     caseNode->breakList);//S.breakList=mergeList(S.breakList, C.breakList)
                if (caseNode->attributes["keyword"] == "C") {
                    tester.push_back(new Quadruples("-", expNode->attributes["addr"], caseNode->attributes["addr"],
                                                    tmp));//T.code+=(tmp=E.addr-C.addr)
                    tester.push_back(new Quadruples("jz", tmp, "",
                                                    caseNode->attributes["label"]));//T.code+=(if tmp==0 goto C.label)
                } else {
                    tester.push_back(
                            new Quadruples("jmp", "", "", caseNode->attributes["label"]));//T.code+=(goto C.label)
                }
            }
    ...
    Sign *iteration_statement() {
        ...
        if (cur_sign == "while") {
            ...
            ret->code.push_back(
                    new Quadruples("jz", node->attributes["addr"], "", exitLabel));//I.code+=(jz, C.addr, $, exitlabel)
            match(")");
            if (!jumpable) {
            ...
            if (expAddr != "") {//if(not(B->ε))
                ...
                ret->code.push_back(new Quadruples("jz", node->attributes["addr"], "",
                                                   exitLabel));//I.code+=(jz, B.addr, $, exitlabel)
            } else {
            ...
            if (!leftv) {
                ...
                if (arrayFlag == "P") {
                    ...
                    if (fieldFlag == "") {
                        ...
                        auto fieldPos = find_if(symbolTable[sub_scope].begin(), symbolTable[sub_scope].end(),
                                                [=](Symbol *s) {
                ...
                if (arrayFlag == "P") {
                    ...
                    if (fieldFlag == "") {
                        ...
                        auto fieldPos = find_if(symbolTable[subScope].begin(), symbolTable[subScope].end(),
                                                [=](Symbol *s) {
                                                    return s->name == fieldFlag;
                                                });
